...
handler.help = ["fastgpt"]; // وصف الأمر
handler.tags = ["ai"]; // الوسوم المرتبطة بالأمر
handler.command = /^(fastgpt)$/i; // الأوامر التي يتم استخدامها لاستدعاء الأمر
...
async function chatWithGPT(your_qus) {
    ...
    const response2 = await fetch("https://chat.aidutu.cn/api/chat-process", {
        ...
        body: JSON.stringify({
            ...
            top_p: 1,
            options: ops,
            systemMessage: "أنت ChatGPT، نموذج لغة كبير تم تدريبه من قبل OpenAI. اتبع تعليمات المستخدم بعناية. استجب باستخدام markdown.",
        }),
    });
