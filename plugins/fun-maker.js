...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    switch (command) {
      ...
      case 'trash':
      case 'basura':
      let img = global.API('lann', '/api/maker/trash', { url: pp }, 'apikey')
      conn.sendFile(m.chat, img, 'img.png', `✅ ${mssg.result}`, m)
      m.react(done)
      break 
      case 'rip':
      let ri = global.API('lann', '/api/maker/rip', { url: pp }, 'apikey')
      conn.sendFile(m.chat, ri, 'img.png', `✅ ${mssg.result}`, m)
      m.react(done)
      break
      case 'shit':
      let sh = global.API('lann', '/api/maker/shit', { url: pp }, 'apikey')
      ...
      let az = global.API('lann', '/api/maker/spank', { url: me, url2: pp }, 'apikey')
      conn.sendFile(m.chat, az, 'img.png', `✅ ${mssg.result}`, m)
      m.react(done)
      break  
      case 'imgdel':
      let del = global.API('lann', '/api/maker/delete', { url: pp }, 'apikey')
      conn.sendFile(m.chat, del, 'img.png', `✅ ${mssg.result}`, m)
      m.react(done)
      break 
      case 'beautiful':
      let bea = global.API('lann', '/api/maker/beautiful', { url: pp }, 'apikey')
