...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
   ...
   let tee = `*أكتب النص الذي تريد تحويله الى لوغو*\n\n📌 Example : *${usedPrefix + command}* JITOSSA`
   let too = `*أكتب النص الذي تريد تحويله الى لوغو*\n\n📌 Example : *${usedPrefix + command}* JITOSSA`
    m.react(rwait)
...
switch (command) {
	...
	case 'logololi':
	if (!text) throw tee
	let img = global.API('lann', '/api/maker/loli', { text }, 'apikey')
	conn.sendFile(m.chat, img, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break 
	case 'neon': 
	if (!text) throw tee
	let ne = global.API('lann', '/api/textpro/neon', { text }, 'apikey')
	conn.sendFile(m.chat, ne, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break 
	case 'devil': 
	if (!text) throw tee
	let de = global.API('lann', '/api/textpro/devil', { text }, 'apikey')
	conn.sendFile(m.chat, de, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break 
	case 'transformer': 
	if (!text) throw tee
	let tra = global.API('lann', '/api/textpro/transformers', { text }, 'apikey')
	conn.sendFile(m.chat, tra, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break 
	case 'thunder': 
	if (!text) throw tee
	let thu = global.API('lann', '/api/textpro/thunder', { text }, 'apikey')
	...
	let [c, d] = text.split`+`
	let gff = global.API('lann', '/api/textpro/graffiti', { text: c, text2: d}, 'apikey')
	conn.sendFile(m.chat, gff, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break
	case 'bpink': 
	if (!text) throw tee
	let bpin = global.API('lann', '/api/textpro/blackpink', { text }, 'apikey')
	conn.sendFile(m.chat, bpin, 'logo.png', `✅${mssg.result}`, m)
	m.react(done)
	break 
	case 'joker': 
	if (!text) throw tee
	let jok = global.API('lann', '/api/textpro/joker', { text }, 'apikey')
	conn.sendFile(m.chat, jok, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break 
	case 'matrix': 
	if (!text) throw tee
	let ma = global.API('lann', '/api/textpro/matrix', { text }, 'apikey')
	conn.sendFile(m.chat, ma, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break 
	case 'wolf': 
   if (!text) throw tee
   let wo = global.API('lann', '/api/textpro/logowolf', { text: 'nansoffc', text2: text}, 'apikey')
	conn.sendFile(m.chat, wo, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break
	case 'glow': 
	if (!text) throw tee
	let glo = global.API('lann', '/api/textpro/advancedglow', { text }, 'apikey')
	...
	if (!text.includes('+')) throw too  
	let [a, b] = text.split`+`   
	let ph = global.API('lann', '/api/textpro/pornhub', { text: a, text2: b}, 'apikey')
	conn.sendFile(m.chat, ph, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break
	case 'ballon': 
	if (!text) throw tee
	let ball = global.API('lann', '/api/textpro/ballon', { text }, 'apikey')
	conn.sendFile(m.chat, ball, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break
	case 'dmd': 
	if (!text) throw tee
	let dm = global.API('lann', '/api/textpro/diamond', { text }, 'apikey')
	conn.sendFile(m.chat, dm, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break
	case 'lightglow': 
	if (!text) throw tee
	let lglo = global.API('lann', '/api/textpro/lightglow', { text }, 'apikey')
	conn.sendFile(m.chat, lglo, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break 
	case 'american': 
	if (!text) throw tee
	let am = global.API('lann', '/api/textpro/American-flag', { text }, 'apikey')
	conn.sendFile(m.chat, am, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break 
	case 'halloween': 
	if (!text) throw tee
	let hall = global.API('lann', '/api/textpro/halloween', { text }, 'apikey')
	conn.sendFile(m.chat, hall, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break 
	case 'green': 
	if (!text) throw tee
	let hgre = global.API('lann', '/api/textpro/green-horror', { text }, 'apikey')
	conn.sendFile(m.chat, hgre, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break 
	case 'glitch': 
	if (!text) throw tee
	let igli = global.API('lann', '/api/textpro/impressive-glitch', { text }, 'apikey')
	conn.sendFile(m.chat, igli, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break
	case 'paper': 
	if (!text) throw tee
	let pap = global.API('lann', '/api/textpro/art-paper-cut', { text }, 'apikey')
	...
	if (!text.includes('+')) throw too  
	let [e, f] = text.split`+`   
	let marv = global.API('lann', '/api/textpro/marvel', { text: e, text2: f}, 'apikey')
	...
	if (!text.includes('+')) throw too  
	let [g, h] = text.split`+`   
	let nin = global.API('lann', '/api/textpro/ninja', { text: g, text2: h}, 'apikey')
	conn.sendFile(m.chat, nin, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break
	case 'future': 
	if (!text) throw tee
	let futu = global.API('lann', '/api/textpro/futuristic', { text }, 'apikey')
	conn.sendFile(m.chat, futu, 'logo.png', `✅ ${mssg.result}`, m)
	m.react(done)
	break
	case '3dbox': 
	if (!text) throw tee
	let box = global.API('lann', '/api/textpro/3dboxtext', { text }, 'apikey')
	...
	if (!text.includes('+')) throw too  
	let [i, j] = text.split`+`
	let gff2 = global.API('lann', '/api/textpro/graffiti2', { text: i, text2: j}, 'apikey')
